...
def load_glue_data(
    ...
    eval_batch_size_per_accum = eval_batch_size // config.experiment.gradient_accumulation_steps
    ...
    def tokenize_function(examples):
        ...
        outputs = tokenizer(*args, truncation=True, max_length=config.model.max_seq_length)
        return outputs
    ...
    eval_dataloader = DataLoader(
        ...
        shuffle=False,
        collate_fn=collate_fn,
        batch_size=eval_batch_size_per_accum,
    )
...
def load_mcqa_data(
    ...
    def tokenize_function(examples):
        ...
        tokenized_examples = tokenizer(
            first_sentences, second_sentences, truncation=True, max_length=config.model.max_seq_length
        )
        tokenized_output = {
    ...
    def tokenize_function_for_causal_lm(examples):
        if task in ["obqa", "arc-c", "arc-e"]:
            ...
            tokenized_output = tokenizer(texts, padding=False, truncation=True, max_length=config.model.max_seq_length)
            tokenized_output["labels"] = [
                label_map[label] for label in examples["answerKey"]
            ]
        elif task == "cqa":
