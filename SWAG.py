...
class SWAG(torch.nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        # select which modules to swag: 'all' (all leaf modules with parameters),
        # 'grad_only' (only modules requiring gradients), 'lora_only' (only lora layers)
        if modules_to_swag not in ["grad_only", "all", "lora_only"]:
            raise Exception(
                'Swag modules must be "grad_only", "lora_only" or "all".')
        ...
        if modules_to_swag == "grad_only":
            self.module_names = [
                name for (name, module) in get_modules_with_grad(self.base)
            ]
        elif modules_to_swag == "lora_only":
            self.module_names = [
                name
                for (name, module) in get_modules_with_grad(self.base)
                if "lora" in name
            ]
    ...
    def is_swag_module(self, name, module):
        if (
            not module._parameters
            or list(module.children())
            or (
                self.modules_to_swag in {"grad_only", "lora_only"}
    ...
    def init_swag_model(self):
        ...
        for name, module in self.base.named_modules():
            ...
            for param_name in list(module._parameters.keys()):
                ...
                param_sum += data.numel()

                module.register_buffer(
                    "%s_mean" % param_name, data.new(data.size()).zero_()
                )
                module.register_buffer(
                    "%s_sq_mean" % param_name, data.new(data.size()).zero_()
                )

                if self.no_cov_mat is False:
                    module.register_buffer(
                        "%s_cov_mat_sqrt" % param_name,
                        data.new_empty((0, data.numel())).zero_(),
                    )
        ...
        self.lora_params = param_sum / 1e3
        print("Learnable params: ", self.lora_params, " K")
    ...
    def sample_blockwise(self, scale, cov, fullrank):
        for module, name in self.params:
            ...
            if cov is True:
                ...
                eps = cov_mat_sqrt.new_empty(
                    (cov_mat_sqrt.size(0), 1)).normal_()
                cov_sample = (
    ...
    def collect_model(self, base_model):
        ...
        # update SWAG model
        for i, ((module, name), base_param) in enumerate(
            zip(self.params, base_model_params)
        ):
            mean = module.__getattr__("%s_mean" % name)
            sq_mean = module.__getattr__("%s_sq_mean" % name)
    ...
    def load_state_dict(self, state_dict, strict=False, assign=False):
        ...
        super(SWAG, self).load_state_dict(
    ...
    def export_numpy_params(self, export_cov_mat=False):
        ...
        for module, name in self.params:
            mean_list.append(module.__getattr__("%s_mean" %
                             name).cpu().numpy().ravel())
            sq_mean_list.append(
                module.__getattr__("%s_sq_mean" % name).cpu().numpy().ravel()
            )
            if export_cov_mat:
                cov_mat_list.append(
                    module.__getattr__("%s_cov_mat_sqrt" %
                                       name).cpu().numpy().ravel()
                )
    ...
    def import_numpy_weights(self, w):
        ...
        for module, name in self.params:
            ...
            module.__setattr__(name, mean.new_tensor(
                w[k: k + s].reshape(mean.shape)))
            k += s
    ...
    def block_logll(self, param_list, mean_list, var_list, cov_mat_root_list):
        ...
        for i, (param, mean, var, cov_mat_root) in enumerate(
            ...
            # print('Block: ', i)
            block_ll = self.compute_ll_for_block(
                param, mean, var, cov_mat_root)
